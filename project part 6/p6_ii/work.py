...
out_path = Path(__file__).resolve().parent/'data'/'test20'/'dev.p6.model.out'
        ...
        with torch.no_grad():
            ...
            with open(out_path, "a+") as file_object: 
                ...
                for count in range(len(words)):
                    file_object.write(words[count] + ' ' + dicto[pred_tags[count]])
                    print(words[count] + ' ' + dicto[pred_tags[count]])
                    file_object.write("\n")
                file_object.write("\n")
